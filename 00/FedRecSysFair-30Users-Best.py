...
class FedCustom(fl.server.strategy.Strategy):
    def __init__(self, fraction_fit: float = 1.0, fraction_evaluate: float = 1.0, 
                 min_fit_clients: int = NUM_CLIENTS, min_evaluate_clients: int = NUM_CLIENTS, min_available_clients: int = NUM_CLIENTS,
                 normalized_variance_min: float = 0.2, normalized_variance_max: float = 0.8,
                 p_diff_loss: float = 0.0, p_group_mean: float = 0.0) -> None:
        super().__init__()
        ...
        self.normalized_variance_min = normalized_variance_min
        self.normalized_variance_max = normalized_variance_max
    ...
    def fairness_regularization(self, server_round, client_index, loss, global_mean_loss, group_mean_loss, global_groups_variance, normalized_variance_min = 0.2, normalized_variance_max = 0.8, p_diff_loss = 1, p_group_mean = 1):
        ...
        if global_groups_variance < min_var:
            normalized_variance = normalized_variance_min 
        elif global_groups_variance > max_var:
            normalized_variance = normalized_variance_max  
        else:
            normalized_range = (global_groups_variance - min_var) / (max_var - min_var)
            normalized_variance = normalized_variance_min + normalized_range * (normalized_variance_max - normalized_variance_min)

        fairness_penalty = diff_loss_global_mean * p_diff_loss + (group_mean_loss * normalized_variance) * p_group_mean

        # Grava os detalhes em fairness_debug.log
        with open('fairness_debug.log', 'a') as log_file:
            ...
            log_file.write(f"diff_loss_global_mean: {diff_loss_global_mean}\n")
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        for client_index, (client, fit_res) in enumerate(results):
            ...
            fairness_loss = self.fairness_regularization(server_round, client_index, local_loss, global_mean_loss, group_mean_loss, global_groups_variance, normalized_variance_min = 0.2, normalized_variance_max = 0.8)
    ...
    def evaluate(self, server_round: int, parameters: Parameters) -> Optional[Tuple[float, Dict[str, Scalar]]]:
        ...
        for result in RESULTS:
            if (result['normalized_variance_min'] == normalized_variance_min and
                result['normalized_variance_max'] == normalized_variance_max and
                result['p_diff_loss'] == p_diff_loss and
                result['p_group_mean'] == p_group_mean):
        ...
        if not config_exists:
            RESULTS.append({
                'normalized_variance_min': normalized_variance_min,
                'normalized_variance_max': normalized_variance_max,
                'p_diff_loss': p_diff_loss,
                'p_group_mean': p_group_mean,
...
normalized_variance_min_list = [0.2]
normalized_variance_max_list = [0.8]
...
for normalized_variance_min in normalized_variance_min_list:
    for normalized_variance_max in normalized_variance_max_list:
        for p_diff_loss in p_diff_loss_list:
            for p_group_mean in p_group_mean_list:

                # Print dos valores das variáveis atuais
                print(f"normalized_variance_min: {normalized_variance_min}, "
                      f"normalized_variance_max: {normalized_variance_max}, "
                      f"p_diff_loss: {p_diff_loss}, "
                      f"p_group_mean: {p_group_mean}")

                strategy = FedCustom(normalized_variance_min=normalized_variance_min, normalized_variance_max=normalized_variance_max,
                                    p_diff_loss=p_diff_loss, p_group_mean=p_group_mean)
                ...
                # Salvar o último valor adicionado na lista RESULTS em results.log
                if RESULTS:
                    last_result = RESULTS[-1]
                    with open('results.log', 'a') as log_file:
                        log_file.write(json.dumps(last_result, indent=4))
