...
class FedCustom(fl.server.strategy.Strategy):
    def __init__(self, fraction_fit: float = 1.0, fraction_evaluate: float = 1.0, 
                 ...
                 p_diff_loss: float = 0.0, p_group_mean: float = 0.0, factor_variance: float = 0.0) -> None:
        super().__init__()
    ...
    def fairness_regularization(self, server_round, client_index, loss, global_mean_loss, group_mean_loss, global_groups_variance, factor_variance = 867.9540957096126):
        ...
        if server_round > 0:
            previous_global_groups_variance = self.previous_global_loss
            global_groups_variance_variation = global_groups_variance - previous_global_groups_variance
        else:
            global_groups_variance_variation = 0  # No primeiro round, não há variação anterior

        # Atualizar a perda global armazenada
        self.previous_global_loss = global_groups_variance

        # Ajustar o fator de penalidade com base na variação da perda
        adaptive_penalty_weight = base_penalty_weight * (1 + decay_rate * global_groups_variance_variation)
        ...
        # with open('FedRecSysFair-FedCustom-Aggregate-Loss-Fair-Activity-Optuna_05_Sqtr-Taxa.log', 'a') as log_file:
        ...
        #     log_file.write(f"scaled_variance: {scaled_variance}\n")
        #     log_file.write(f"adaptive_penalty_weight: {adaptive_penalty_weight}\n")
        #     log_file.write(f"fairness_penalty: {fairness_penalty}\n")
        #     log_file.write(f"loss + fairness_penalty: {loss + fairness_penalty}\n")
        
        # Retornar o loss ajustado com a penalidade de justiça
...
def objective(trial):
    ...
    factor_variance = trial.suggest_float("factor_variance", 1, 10000)
