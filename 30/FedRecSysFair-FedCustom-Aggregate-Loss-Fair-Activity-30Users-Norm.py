...
for nv in normalized_variance_tests:
    ...
    results[nv] = metrics['RgrpActivity'] 
    print(f"Final RgrpActivity for normalized_variance {nv}: {metrics['RgrpActivity'] }")

    # Comparar para selecionar o melhor valor
    if metrics['RgrpActivity'] < best_rgrp_activity:
        best_rgrp_activity = metrics['RgrpActivity']
        best_variance = nv
