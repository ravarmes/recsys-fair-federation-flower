...
# Defina a lista de valores de normalized_variance para teste
normalized_variance_tests = [0.1, 0.3]

NUM_ROUNDS = 2
NUM_CLIENTS = 30

# Inicializa estrutura para armazenar resultados de todas as simulações
results = {nv: [] for nv in normalized_variance_tests}

best_rgrp_activity = float('inf')
best_variance = None

class FedCustomWithLogging(FedCustom):
    def aggregate_evaluate(self, server_round, results, failures):
        # Chama o método pai para agregar
        loss_aggregated, metrics_aggregated = super().aggregate_evaluate(server_round, results, failures)

        # Adiciona o valor de RgrpActivity ao resultado da configuração atual
        for metric in metrics_aggregated:
            if 'RgrpActivity' in metric:
                # Supondo que 'RgrpActivity' seja uma métrica e está estruturado corretamente
                results.append(metrics_aggregated[metric])  # Armazena diretamente na lista de resultados
...
for nv in normalized_variance_tests:
    ...
    strategy = FedCustomWithLogging(normalized_variance=nv)
...
# Após a simulação, avaliar apenas o último round para avaliação de resultado
for nv in normalized_variance_tests:
    if results[nv]:  # Apenas considere se a lista não está vazia
        final_rgrp_activity = results[nv][-1]  # Pegue o último valor armazenado
        print(f"Final RgrpActivity for normalized_variance {nv}: {final_rgrp_activity}")

        if final_rgrp_activity < best_rgrp_activity:
