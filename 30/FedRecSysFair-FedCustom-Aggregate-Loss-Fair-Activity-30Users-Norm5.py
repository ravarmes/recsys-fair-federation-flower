...
# Define o número de rounds a serem utilizados
NUM_ROUNDS = 2

# Testar diferentes valores de normalized_variance
# normalized_variance_tests = [0.1, 0.3, 0.5, 0.7, 1.0]
normalized_variance_tests = [0.1]
best_rgrp_activity = float('inf')
best_variance = None
results = {nv: [] for nv in normalized_variance_tests}  # Inicializa estruturas para todos os NVs

for nv in normalized_variance_tests:
    print(f"Testing normalized_variance = {nv}")
    strategy = FedCustom(normalized_variance=nv)

    # A estrutura para armazenar RgrpActivity em cada rodada
    for round_num in range(NUM_ROUNDS):
        fl.simulation.start_simulation(
            client_fn=client_fn,
            num_clients=NUM_CLIENTS,
            config=fl.server.ServerConfig(num_rounds=round_num + 1),  # Incrementando o número de rounds
            strategy=strategy,
            client_resources=client_resources,
        )

        loss, metrics = strategy.evaluate(server_round=round_num + 1, parameters=strategy.initialize_parameters(None))
        
        print("\n\n----------------------------------")
        print(metrics)
        print("----------------------------------\n\n")

        # Armazenar o valor de RgrpActivity
        rgrp_activity_current = metrics.get('RgrpActivity', None)
        if rgrp_activity_current is not None:
            results[nv].append(rgrp_activity_current)  # Armazena todos os valores de RgrpActivity em cada round

        # Imprimir o valor atual de RgrpActivity
        print(f"Round {round_num + 1} RgrpActivity for normalized_variance {nv}: {rgrp_activity_current}")

    # Verifique o último valor de RgrpActivity para comparação
    final_rgrp_activity = results[nv][-1] if results[nv] else None

    # Comparar para selecionar o melhor valor
    if final_rgrp_activity is not None and final_rgrp_activity < best_rgrp_activity:
        best_rgrp_activity = final_rgrp_activity
        best_variance = nv
