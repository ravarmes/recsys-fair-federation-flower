    ...
    def evaluate(self, parameters, config):
        ...
        return (
            (float(loss), len(self.valloader), 
            {
                ...
                "RgrpActivity": RgrpActivity  # Incluindo RgrpActivity nas métricas
            })
        )
...
# Dados de configuração, classes e funções são presumidos como definidos anteriormente
...
best_variance = None
results = {nv: [] for nv in normalized_variance_tests}  # Inicializa listas para RgrpActivity
...
for nv in normalized_variance_tests:
    ...
    # A avaliação ao final da simulação
    loss, metrics = strategy.evaluate(server_round=NUM_ROUNDS, parameters=strategy.initialize_parameters(None))
    
    # Print para entender a estrutura de metrics
    print(f"Metrics after evaluation: {metrics}")

    # Verificar se RgrpActivity é uma lista de tuplas ou um valor escalar
    if 'RgrpActivity' in metrics:
        rgrp_activity_value = metrics['RgrpActivity']
        if isinstance(rgrp_activity_value, (list, tuple)):
            results[nv] = [value[1] for value in rgrp_activity_value]  # Se for uma lista de tuplas, pega os segundos valores
        else:
            results[nv] = [rgrp_activity_value]  # Armazene o valor escalar em uma lista

        print(f"Final RgrpActivity for normalized_variance {nv}: {results[nv]}")

    # Avalia o último valor para comparação
    if results[nv]:
        final_rgrp_activity = results[nv][-1]  # Pega o último valor armazenado
