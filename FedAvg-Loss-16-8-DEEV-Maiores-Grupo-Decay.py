...
def load_datasets(num_clients: int, filename: str, seed: int = 42):
    ...
    for cliente_id in sorted(cliente_avaliacoes.keys()):
        ...
        batch_size = 16 if cliente_id <= 14 else 8  # Definindo o tamanho do lote de acordo com o nível de atividade dos usuários
...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        # Encontrar o grupo com a maior perda média
        group_max_loss = max(group_losses, key=group_losses.get)
        ...
        # Filtrar clientes que estão acima da média global e pertencem ao grupo com maior perda média
        filtered_results = [
            (client, fit_res) for idx, (client, fit_res) in enumerate(results) 
            if fit_res.metrics.get('loss', 0) > average_loss and idx in G_ACTIVITY[group_max_loss]
        ]
