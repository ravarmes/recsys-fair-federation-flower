...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        """Agrega os parâmetros dos modelos treinados pelos clientes considerando apenas perdas menores que a média."""
        ...
        # Filtrar clientes com perda menor que a média
        filtered_results = [(client, fit_res) for client, fit_res in results if fit_res.metrics.get('loss', 0) < average_loss]

        print(f"Média das perdas: {average_loss}")
        print(f"Quantidade de clientes considerados: {len(filtered_results)} de {num_clients}")

        if not filtered_results:
            print("Nenhum cliente com perda menor que a média.")
            return None, {}
