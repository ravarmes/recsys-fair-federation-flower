...
def load_datasets(num_clients: int, filename: str, seed: int = 42):
    ...
    for cliente_id in sorted(cliente_avaliacoes.keys()):
        ...
        batch_size = 18 if cliente_id <= 14 else 8  # Definindo o tamanho do lote de acordo com o nível de atividade dos usuários
...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        average_loss = total_loss / num_clients if num_clients > 0 else 0

        print(f"Média das perdas: {average_loss}")
        
        # Calcula a perda média para cada grupo
        group_losses = {}
        for group, client_indexes in G_AGE.items():
            group_loss = sum(fit_res.metrics.get('loss', 0) for idx, (client, fit_res) in enumerate(results) if idx in client_indexes)
            group_count = sum(1 for idx in client_indexes if idx < num_clients)
            group_losses[group] = group_loss / group_count if group_count > 0 else 0

        print(f"Perdas médias por grupo: {group_losses}")

        from heapq import nlargest

        # Encontrar os três grupos com as maiores perdas médias
        top_n_groups = nlargest(3, group_losses, key=group_losses.get)

        # Calcular a fração de clientes a serem selecionados com decaimento
        fraction_of_clients = 1 - (0.01 * (server_round - 1))
        fraction_of_clients = max(fraction_of_clients, 0)  # Não permitir que a fração fique negativa
        print(f"Fração de clientes a serem selecionados: {fraction_of_clients:.2f}")

        # Filtrar clientes que estão acima da média global e pertencem aos três grupos com maiores perdas médias
        filtered_results = [
            (client, fit_res) for idx, (client, fit_res) in enumerate(results)
            if fit_res.metrics.get('loss', 0) > average_loss and any(idx in G_AGE[group] for group in top_n_groups)
        ]

        # Determinar o número de clientes a serem selecionados baseado na fração
        num_selected_clients = int(len(filtered_results) * fraction_of_clients)
        filtered_results = filtered_results[:num_selected_clients]

        print(f"Quantidade de clientes considerados: {len(filtered_results)} de {num_clients}")

        if not filtered_results:
            print("Nenhum cliente com perda acima da média nos três grupos com maiores perdas.")
            return None, {}
        ...
        if not filtered_results:
            print("Nenhum cliente com perda acima da média no grupo com maior perda.")
            return None, {}

        # Recalcular total_loss com apenas os clientes filtrados
        filtered_total_loss = sum(fit_res.metrics.get('loss', 0) for _, fit_res in filtered_results)
        
        weights_results = [(parameters_to_ndarrays(fit_res.parameters), fit_res.metrics.get('loss', 0)) for _, fit_res in filtered_results]
        ...
        metrics_aggregated = {}
        for client_index, (client, fit_res) in enumerate(filtered_results):
            loss = fit_res.metrics.get('loss', 0)
            weight = loss / filtered_total_loss if filtered_total_loss != 0 else 0
...
fl.simulation.start_simulation(
    ...
    config=fl.server.ServerConfig(num_rounds=48),
    strategy=FedCustom(),
