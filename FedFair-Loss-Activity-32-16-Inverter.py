...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def fairness_regularization(self, server_round, client_index, loss, group_mean_loss, global_groups_variance):
        ...
        with open("FedFair-Loss-Activity-32-16-Inverter.log", "a") as log_file:
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        def aggregate(results: List[Tuple[NDArrays, float]]) -> NDArrays:
            # Calcula a perda total durante o treinamento
            loss_total = sum(1 / loss for (_, loss) in results)

            # Crie uma lista de pesos, agora multiplicados pelo inverso da perda
            weighted_weights = [
                [layer * (1 / loss) for layer in weights] for weights, loss in results
            ]
