...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def fairness_regularization(self, server_round, client_index, loss, group_mean_loss, global_groups_variance, loss_avg_per_group, client_group_index):
        ...
        global_mean_loss = sum(loss_avg_per_group) / len(loss_avg_per_group)
        fairness_penalty = (group_mean_loss - global_mean_loss) * (normalized_variance)

        # Logging das informações e ajustes
        with open("FedRecSysFair-FedCustom-Aggregate-Loss-Fair-Activity-Taxa7_debug.log", "a") as log_file:
