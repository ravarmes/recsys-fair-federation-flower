...
class FedCustom(Strategy):
    ...
    # Agregando pelo número de exemplos processados pelo cliente (INVERTIDO)
    # Ou seja, clientes do grupo favorecido (com mais exemplos de avaliações contribuem menos)
    def aggregate_fit(
        self,
        server_round: int,
        ...
        # Convert results to a list of arrays and associated weights (contributions) using inverse of num_examples as weights
        weights_results = [
        ...
        for client_index, (client, fit_res) in enumerate(results):
            ...
            weight = 1 / num_examples  # Calculating the weight as inverse of number of examples
            print(f"Cliente {client.cid}: Número de exemplos = {num_examples}, Peso = {weight}")
