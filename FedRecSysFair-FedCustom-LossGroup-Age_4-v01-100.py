...
class FedCustom(Strategy):
    ...
    def configure_fit(
        ...
        configurations = [
            {"server_round": server_round, "local_epochs": 16, "learning_rate": 0.01, "lotes_por_rodada": lotes_por_rodada},
            {"server_round": server_round, "local_epochs": 17, "learning_rate": 0.01, "lotes_por_rodada": lotes_por_rodada},
            {"server_round": server_round, "local_epochs": 18, "learning_rate": 0.01, "lotes_por_rodada": lotes_por_rodada},
            {"server_round": server_round, "local_epochs": 20, "learning_rate": 0.01, "lotes_por_rodada": lotes_por_rodada},
            {"server_round": server_round, "local_epochs": 22, "learning_rate": 0.01, "lotes_por_rodada": lotes_por_rodada},
            {"server_round": server_round, "local_epochs": 24, "learning_rate": 0.01, "lotes_por_rodada": lotes_por_rodada},
            {"server_round": server_round, "local_epochs": 25, "learning_rate": 0.01, "lotes_por_rodada": lotes_por_rodada},
            {"server_round": server_round, "local_epochs": 26, "learning_rate": 0.01, "lotes_por_rodada": lotes_por_rodada}
        ]
        ...
        # Verificar se sorted_groups não está vazio
        if not sorted_groups:
            print("No loss data available, assigning config_g2 to all clients.")
            return [(client, FitIns(parameters, configurations[1])) for client in clients]

        fit_configurations = []

        # Função auxiliar para ordenar clientes por perda
        def get_client_loss(client):
            return self.all_losses[int(client.cid)] if int(client.cid) < len(self.all_losses) else float('inf')

        G_AGE = {1: [14, 132, 194, 262, 273], 2: [8, 23, 26, 33, 48, 50, 61, 64, 70, 71, 76, 82, 86, 90, 92, 94, 96, 101, 107, 124, 126, 129, 134, 140, 149, 157, 158, 159, 163, 168, 171, 174, 175, 189, 191, 201, 207, 209, 215, 216, 222, 231, 237, 244, 246, 251, 255, 265, 270, 275, 282, 288, 290], 3: [3, 6, 7, 9, 10, 11, 15, 16, 21, 22, 24, 28, 29, 31, 32, 34, 35, 37, 39, 40, 41, 42, 43, 44, 45, 51, 53, 55, 56, 59, 60, 63, 65, 66, 69, 72, 73, 74, 75, 79, 80, 81, 85, 89, 93, 97, 102, 103, 104, 106, 108, 109, 110, 111, 116, 118, 119, 120, 122, 128, 130, 131, 133, 135, 136, 138, 139, 141, 142, 143, 145, 147, 151, 155, 161, 164, 169, 170, 173, 176, 179, 181, 183, 186, 187, 188, 190, 192, 193, 195, 196, 198, 200, 202, 203, 204, 205, 206, 211, 212, 213, 217, 219, 220, 223, 225, 226, 229, 230, 232, 233, 234, 236, 238, 240, 241, 249, 252, 253, 254, 258, 260, 261, 264, 267, 268, 269, 276, 277, 279, 280, 283, 285, 286, 287, 289, 291, 293, 294, 295, 296, 298], 4: [1, 2, 4, 5, 13, 17, 18, 25, 27, 36, 38, 49, 52, 57, 68, 77, 78, 84, 87, 88, 91, 95, 98, 99, 100, 105, 112, 117, 121, 127, 144, 146, 150, 152, 153, 156, 166, 172, 177, 182, 199, 208, 210, 214, 227, 228, 243, 245, 248, 250, 256, 263, 271, 272, 278, 292, 297, 299], 5: [19, 20, 30, 46, 47, 54, 58, 62, 67, 83, 113, 125, 137, 148, 160, 165, 167, 184, 197, 221, 235, 239, 242, 281], 6: [0, 114, 115, 123, 178, 180, 185, 224, 247, 257, 266, 274], 7: [12, 154, 162, 218, 259, 284]}

        # Configurar clientes de acordo com as perdas dos grupos
        for i, (group, _) in enumerate(sorted_groups):
            if group in G_AGE:
                ...
                mid_index = len(group_clients_sorted_by_loss) // 1

                # Proporção de 50% para configuração
                high_config = configurations[min(i + 2, 7)]
                low_config = configurations[min(i + 1, 7)]

                for client in group_clients_sorted_by_loss[:mid_index]:
                    fit_configurations.append((client, FitIns(parameters, low_config)))
                for client in group_clients_sorted_by_loss[mid_index:]:
                    fit_configurations.append((client, FitIns(parameters, high_config)))

        # Garantir que todos os clientes estejam configurados, adicionando qualquer cliente restante
        configured_clients = {client for client, _ in fit_configurations}
        remaining_clients = [client for client in clients if client not in configured_clients]
        for client in remaining_clients:
            fit_configurations.append((client, FitIns(parameters, configurations[1])))
