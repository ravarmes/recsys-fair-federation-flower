...
class FedCustom(Strategy):
    ...
    # Função de regularização de equidade
    def fairness_regularization(self, loss, group_mean_loss, group_variance, lambda_fairness):
        # Calcula a diferença entre o erro local do cliente e o erro médio do grupo
        diff_loss_group_mean = loss - group_mean_loss
        
        # Calcula a penalidade de equidade baseada na diferença entre o erro do cliente e o erro médio do grupo
        fairness_penalty = lambda_fairness * diff_loss_group_mean
        
        # Retorna a perda local do cliente com a penalidade de equidade aplicada
    ...
    def configure_fit(
        ...
        # Configuração única de treinamento para todos os clientes
        config = {
    ...
    def aggregate_fit(
        ...
        # Calcular a variância entre as perdas dos grupos
        group_variance = np.var(list(self.loss_avg_per_group.values()))
        
        # Aplicar a regularização de equidade e calcular pesos
        fairness_losses = []
        for client_index, (client, fit_res) in enumerate(results):
            local_loss = fit_res.metrics.get('loss', 0)
            
            # Determinar a qual grupo o cliente pertence
            client_group = next(group for group, client_indexes in G_GENDER.items() if client_index in client_indexes)
            group_mean_loss = self.loss_avg_per_group[client_group]
            
            # Aplicar a regularização de equidade
            fairness_loss = self.fairness_regularization(local_loss, group_mean_loss, group_variance, lambda_fairness=0.2)
