...
class FedCustom(Strategy):
    ...
    def configure_fit(
        ...
        if not self.loss_avg_per_group:  # Caso não tenha a perda média, use configuração padrão
            self.loss_avg_per_group = {group: 1.0 for group in G_GENDER}

        fit_configurations = []
        for client in clients:
            # Determine o grupo do cliente
            group = next((grp for grp, clients in G_GENDER.items() if int(client.cid) in clients), None)
            
            # Ajusta os hiperparâmetros com base na perda média do grupo
            if group is not None and group in self.loss_avg_per_group:
                group_loss_avg = self.loss_avg_per_group[group]
                
                # Ajuste dinâmico dos hiperparâmetros
                learning_rate = default_learning_rate / group_loss_avg   
                local_epochs = int(default_local_epochs * group_loss_avg)
                
                # Limitar ajustes extremos
                learning_rate = max(min(learning_rate, 0.1), 0.001)
                local_epochs = max(min(local_epochs, 30), 5)
            else:
    ...
    def aggregate_fit(
        ...
        G_GENDER = {1: [...], 2: [...]}  # Adicione seus grupos aqui

        group_losses = {group: 0.0 for group in G_GENDER}
        group_counts = {group: 0 for group in G_GENDER}

        for index, (client, fit_res) in enumerate(results):
            group = next((grp for grp, clients in G_GENDER.items() if int(client.cid) in clients), None)
            if group is not None:
                group_losses[group] += fit_res.metrics.get('loss', 0)
        ...
        total_loss_avg = sum(group_losses.values()) / max(sum(group_counts.values()), 1)

        client_weights = []
        for index, (client, fit_res) in enumerate(results):
            group = next((grp for grp, clients in G_GENDER.items() if int(client.cid) in clients), None)
            if group is not None:
                weight = (1 - abs(self.loss_avg_per_group[group] - total_loss_avg) / total_loss_avg) if total_loss_avg != 0 else 0
                client_weights.append(weight)

        weights_results = [
            (parameters_to_ndarrays(fit_res.parameters), weight)
            for (client, fit_res), weight in zip(results, client_weights)
        ]

        parameters_aggregated = ndarrays_to_parameters(aggregate(weights_results))
        metrics_aggregated = {"average_loss": total_loss_avg, "loss_avg_per_group": self.loss_avg_per_group}
