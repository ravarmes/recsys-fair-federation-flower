...
class FedCustom(Strategy):
    ...
    def aggregate_fit(
        ...
        def aggregate(weights: List[Tuple[List[np.ndarray], float]]) -> List[np.ndarray]:
            ...
            # Initialize weighted weights
            num_layers = len(weights[0][0])  # Assuming all models have the same number of layers
            weighted_weights = [np.zeros_like(weights[0][0][0]) for _ in range(num_layers)]
