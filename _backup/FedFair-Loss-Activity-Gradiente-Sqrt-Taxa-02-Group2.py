...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def fairness_regularization(self, server_round, client_index, loss, group_mean_loss, global_groups_variance):
        if loss < group_mean_loss:
            adjustment_factor = 0.8  # Recompensa pela contribuição menor que a média
        else:
            adjustment_factor = 1.2  # Penaliza pela contribuição maior
        ...
        with open("FedFair-Loss-Activity-Gradiente-Sqrt-Taxa-02-Group2.log", "a") as log_file:
