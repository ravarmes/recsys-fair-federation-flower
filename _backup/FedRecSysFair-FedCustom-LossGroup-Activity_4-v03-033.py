...
# Dentro de cada grupo, os usuários são ordenados em ordem de perda (33%-67%) // 3
...
class FedCustom(Strategy):
    ...
    def configure_fit(
        ...
        if not hasattr(self, 'loss_avg_per_group'):
            self.loss_avg_per_group = {group: 0 for group in G_ACTIVITY.keys()}
        ...
        if not sorted_groups:
            ...
            return [(client, FitIns(parameters, config_g2)) for client in clients]

        lowest_loss_group = sorted_groups[0][0]
        highest_loss_group = sorted_groups[-1][0]
