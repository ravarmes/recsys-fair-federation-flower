...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def fairness_regularization(self, server_round, client_index, loss, group_mean_loss, global_groups_variance, loss_avg_per_group, client_group_index):
        # Parâmetros de controle de variância
        min_var = 0.00001
        max_var = 0.00100
        min = 0.5
        max = 1.0

        # Ajuste da variância normalizada
        if global_groups_variance < min_var:
            ...
            normalized_variance = min + normalized_range * (max - min)

        # Calcula a média das perdas de todos os grupos
        total_mean_loss = sum(loss_avg_per_group) / len(loss_avg_per_group)

        # Calcula a diferença entre a perda média de cada grupo e a média total
        group_difference = loss_avg_per_group[client_group_index] - total_mean_loss

        # Ajuste proporcional à diferença da média do grupo e da média global
        fairness_adjustment = -group_difference * normalized_variance

        # Penalidade de justiça ajustada
        fairness_penalty = 0
        total_adjustment = fairness_penalty + fairness_adjustment

        # Logging das informações e ajustes
        with open("FedRecSysFair-FedCustom-Aggregate-Loss-Fair-Activity-Taxa5_debug.log", "a") as log_file:
            ...
            log_file.write(f"group_difference: {group_difference}\n")
            log_file.write(f"fairness_penalty: {fairness_penalty}\n")
            log_file.write(f"fairness_adjustment: {fairness_adjustment}\n")
            log_file.write(f"total_adjustment: {total_adjustment}\n")
            log_file.write(f"loss + total_adjustment: {loss + total_adjustment}\n")

        return loss + total_adjustment
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        for client_index, (client, fit_res) in enumerate(results):
            ...
            fairness_loss = self.fairness_regularization(server_round, client_index, local_loss, group_mean_loss, global_groups_variance, self.loss_avg_per_group, group_id)
