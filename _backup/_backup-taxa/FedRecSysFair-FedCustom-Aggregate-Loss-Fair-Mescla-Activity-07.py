...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        weights_results = [ (parameters_to_ndarrays(fit_res.parameters), fit_res.num_examples, fit_res.metrics.get('loss', 0)) for _, fit_res in results ]

        def aggregate_weighted(results: List[Tuple[NDArrays, int, float]], 
                       lambda_val: float) -> NDArrays:
            # Assert that lambda_val is between 0 and 1
            assert 0 <= lambda_val <= 1, "lambda_val must be between 0 and 1"
            
            # Separate weights, num_examples, and losses from results
            weights_list = [weights for weights, _, _ in results]
            num_examples = [num for _, num, _ in results]
            losses = [loss for _, _, loss in results]
            
            # Strategy 1: Compute median
            median_w: NDArrays = [
                np.median(np.asarray(layer), axis=0) for layer in zip(*weights_list)
            ]
            
            # Strategy 2: Compute loss weighted average
            loss_total = sum(losses)
            weighted_weights = [
                [layer * loss for layer in weights] for weights, loss in zip(weights_list, losses)
            ]
            ...
            # Strategy 3: Combine both strategies using lambda_val
            combined_weights: NDArrays = [
                lambda_val * median_layer + (1 - lambda_val) * avg_layer
                for median_layer, avg_layer in zip(median_w, weights_prime)
            ]
            
            return combined_weights

        parameters_aggregated = ndarrays_to_parameters(aggregate_weighted(weights_results, lambda_val=0.7))
