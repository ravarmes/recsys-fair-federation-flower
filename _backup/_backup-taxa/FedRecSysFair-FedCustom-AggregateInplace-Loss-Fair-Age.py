...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        weights_results = [
            (parameters_to_ndarrays(fit_res.parameters), fit_res) for _, fit_res in results
        ]
        ...
        def aggregate_inplace(results: List[Tuple[NDArrays, FitRes]]) -> NDArrays:
            ...
            loss_total = sum(fit_res.metrics['loss'] for (_, fit_res) in results)

            # Compute scaling factors for each result
            scaling_factors = [
                fit_res.metrics['loss'] / loss_total for _, fit_res in results
            ]
        ...
        parameters_aggregated = ndarrays_to_parameters(aggregate_inplace(weights_results))
