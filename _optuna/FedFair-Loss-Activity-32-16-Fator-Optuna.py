...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def fairness_regularization(self, server_round, client_index, loss, group_mean_loss, global_groups_variance, scale):
        
        fairness_penalty = (group_mean_loss) * (global_groups_variance ** 0.5) * scale

        with open("FedFair-Loss-Activity-32-16-Fator-Optuna.log", "a") as log_file:
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        for client_index, (client, fit_res) in enumerate(results):
            ...
            fairness_loss = self.fairness_regularization(server_round, client_index, local_loss, group_mean_loss, global_groups_variance, self.scale)
...
study.optimize(objective, n_trials=300)  # Define o número de iterações (trials)
