...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def fairness_regularization(self, server_round, client_index, loss, group_mean_loss, global_groups_variance, scale_group_mean, scale_groups_variance):
        ...
        round_factor = 1 + (server_round / NUM_ROUNDS)
        fairness_penalty = (group_mean_loss * scale_group_mean) + ((global_groups_variance ** 0.5) * scale_groups_variance)

        with open("FedFair-Loss-Activity-32-16-Fator-Soma-Optuna-300-Round.log", "a") as log_file:
            ...
            log_file.write(f"round_factor: {round_factor}\n")
            log_file.write(f"fairness_penalty: {fairness_penalty}\n")
            log_file.write(f"loss + (fairness_penalty * round_factor): {loss + (fairness_penalty * round_factor)}\n")

        return loss + (fairness_penalty * round_factor)
...
# Best is trial 00 with value: 0.00024338382467577508. Melhores parâmetros encontrados: {'scale_group_mean': 20.248980959523667, 'scale_groups_variance': 38.42112431395835}
# Best is trial 32 with value: 0.00020407403689521640. Melhores parâmetros encontrados: {'scale_group_mean': 0.016226499412696604, 'scale_groups_variance': 4.715585222554768}
