...
class FedCustom(fl.server.strategy.Strategy):
    ...
    def fairness_regularization(self, server_round, client_index, loss, global_mean_loss, group_mean_loss, global_groups_variance, scale_value, decay_rate):
        # Cálculo do scale_value ajustado com o decaimento
        decayed_scale_value = scale_value * (decay_rate ** server_round)
        
        # Calcula a penalidade de justiça usando o scale_value com decaimento
        fairness_penalty = group_mean_loss * global_groups_variance * decayed_scale_value
        
        # Ajusta a perda
        adjusted_loss = loss + fairness_penalty

        with open("FedRecSysFair-300Users-Optuna_10_Gradiente_Sqrt-Activity-Scale-Taxa.log", "a") as log_file:
            ...
            log_file.write(f"scale_value: {scale_value}\n")
            log_file.write(f"decay_rate: {decay_rate}\n")
            log_file.write(f"decayed_scale_value: {decayed_scale_value}\n")
    ...
    def aggregate_fit(self, server_round: int, results: List[Tuple[ClientProxy, FitRes]], failures: List[Union[Tuple[ClientProxy, FitRes], BaseException]]) -> Tuple[Optional[Parameters], Dict[str, Scalar]]:
        ...
        for client_index, (client, fit_res) in enumerate(results):
            ...
            fairness_loss = self.fairness_regularization(server_round, client_index, local_loss, global_mean_loss, group_mean_loss, global_groups_variance, self.scale_value, self.decay_rate)
